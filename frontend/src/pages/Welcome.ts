...
export default class WelcomePage extends Page {
  playButton = new Paragraph("Play →").withId("play-btn")
    .class("font-bold text-2xl mt-10 transition duration-1300 select-none")
    .class(KEY1.join(" "));
  ...
  titleCard = new Div([
    ...
    .class("transition duration-1300 flex flex-col items-center")
    .class(KEY1.join(" "))
    .withId("title-card");
  ...
  bindEvents() {
    ...
    this.playButton.byId()?.addEventListener('click', cb);
  }
}
