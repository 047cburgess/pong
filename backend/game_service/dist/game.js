...
export class Game {
    ...
    gameTick() {
        ...
        if (this.state.pauseCd) {
            ...
            if (this.state.pauseCd === 0) {
                this.state.ball.vel = new Vec2(Math.random() * 2 - 1, Math.random() * 2 - 1).normalize();
            }
        }
        else {
            this.state.tick++;
    ...
    updateBall() {
        ...
        ball.pos = ball.pos.add(ball.vel.mul(this.tickMs * this.params.ballSpeed));
    ...
    collision(side) {
        ...
        const rpos = ball.pos.rot(angle);
        let rvel = ball.vel.rot(angle);
        let pside = side;
        if (side === 1) {
            pside = 2;
        }
        ...
        if (this.state.players[pside]?.health) {
            rvel = this.playerCollision(rpos, rvel, pside);
        }
        if (rpos.x >= FIELD_HALFSIZE) {
            rpos.x = 2 * FIELD_HALFSIZE - rpos.x;
            rvel.x *= -1;
        }
        ball.pos = rpos.rot((360 - angle));
        ball.vel = rvel.rot((360 - angle));
    }
    playerCollision(rpos, rvel, side) {
        const pst = this.state.players[side];
        pst.hitBy = undefined;
        if (rpos.x < FIELD_HALFSIZE) {
            return rvel;
        }
        ...
        if (Math.abs(hdist) > 1) {
            if (typeof this.state.ball.lastRefl === 'number') {
                const lastHit = this.state.players[this.state.ball.lastRefl];
